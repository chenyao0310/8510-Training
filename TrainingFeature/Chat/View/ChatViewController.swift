...
class ChatViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        bindViewModel()
        scrollToBottom(row: IndexPath(row: (viewModel.chatHistory.count) - 1, section: 0), animated: false)
    }
}
...
// MARK: - BindingViewModel
    ...
    private func bindViewModel() {
        viewModel.onOwnerUsersChanged = { [weak self] in
            let lastTwoIndexPath = IndexPath(row: (self?.viewModel.chatHistory.count ?? 0) - 2, section: 0)
                let newIndex = IndexPath(row: (self?.viewModel.chatHistory.count ?? 0) - 1, section: 0)
    ...
    @objc private func stickerToggle() {
       ...
       self.stickerConstraint.constant = self.viewModel.isShowStickers ? 200 : 0
        ...
        UIView.animate(withDuration: 0.2) {
            if self.viewModel.chatHistory.count > 0 {
                self.tableView.scrollToRow(at: IndexPath(row: self.viewModel.chatHistory.count - 1 , section: 0), at: .top, animated: false)
            }
        }
   }
